package com.iconiz.tokenunion.widget.header.waterdrop;
...
public class WaterDropView extends View {
    ...
    public WaterDropView(Context context) {
        ...
        mPaint.setColor(Color.parseColor("#F0CDA5"));
        mMaxCircleRadius = dip2px(20);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        final View thisView = this;
        //宽度：上圆和下圆的最大直径
        final int width = mMaxCircleRadius * 2;
        //高度：上圆半径 + 圆心距 + 下圆半径
        final int height = (int) Math.ceil(bottomCircle.y + bottomCircle.radius);
...
//        canvas.drawCircle(bottomCircle.x, bottomCircle.y, bottomCircle.radius, mPaint);
    ...
    private void makeBezierPath() {
        ...
        if (bottomCircle.y > topCircle.y + dip2px(1)) {
...
//        updateCompleteState(Math.max(0, 1f * (offset - space) / (maxHeight - space)));
    ...
    public void updateCompleteState(int height) {
        ...
        if (height < space) {
            ...
            float y = (float) (limit * (1 - Math.pow(100, -x / dip2px(200))));
